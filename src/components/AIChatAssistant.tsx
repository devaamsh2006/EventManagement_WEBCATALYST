...
export default function AIChatAssistant({
  ...
  const [eventsData, setEventsData] = useState<any[]>([]); // store DB events
  ...
  // ✅ Fetch events from /api/events when chat opens
  useEffect(() => {
    if (isOpen) {
      const fetchEvents = async () => {
        try {
          const res = await fetch("/api/events?limit=20&isActive=true");
          if (!res.ok) throw new Error("Failed to fetch events");
          const data = await res.json();
          setEventsData(data);
        } catch (error) {
          console.error("Error loading events:", error);
          toast.error("Could not load events from database.");
        }
      };
  ...
  // ✅ Send message to AI API with events
    ...
    try {
      const res = await fetch("/api/assistant", {
        ...
        body: JSON.stringify({
          ...
          events: eventsData, // 🔑 send live DB events
        }),
      });
